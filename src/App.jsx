...
const inpSt=new Proxy({},{get(*,p){return getInpSt()[p]},ownKeys(){return Object.keys(getInpSt())},getOwnPropertyDescriptor(*,p){const v=getInpSt();if(p in v)return{configurable:true,enumerable:true,value:v[p]};},has(*,p){return p in getInpSt()}});
const lblSt=new Proxy({},{get(*,p){return getLblSt()[p]},ownKeys(){return Object.keys(getLblSt())},getOwnPropertyDescriptor(*,p){const v=getLblSt();if(p in v)return{configurable:true,enumerable:true,value:v[p]};},has(*,p){return p in getLblSt()}});
const btnPrim=new Proxy({},{get(*,p){return getBtnPrim()[p]},ownKeys(){return Object.keys(getBtnPrim())},getOwnPropertyDescriptor(*,p){const v=getBtnPrim();if(p in v)return{configurable:true,enumerable:true,value:v[p]};},has(*,p){return p in getBtnPrim()}});
const btnGhost=new Proxy({},{get(*,p){return getBtnGhost()[p]},ownKeys(){return Object.keys(getBtnGhost())},getOwnPropertyDescriptor(*,p){const v=getBtnGhost();if(p in v)return{configurable:true,enumerable:true,value:v[p]};},has(*,p){return p in getBtnGhost()}});
const btnDanger=new Proxy({},{get(*,p){const base=getBtnGhost();const v={…base,borderColor:K.cr,color:K.cr};return v[p];},ownKeys(){return Object.keys({…getBtnGhost(),borderColor:””,color:””})},getOwnPropertyDescriptor(*,p){const v={…getBtnGhost(),borderColor:K.cr,color:K.cr};if(p in v)return{configurable:true,enumerable:true,value:v[p]};},has(_,p){return p in {…getBtnGhost(),borderColor:””,color:””}}});
...
<div style={{display:“grid”,gridTemplateColumns:“repeat(7,minmax(70px,1fr))”}}>
{Array.from({length:fd},function(*,i){return <div key={“e”+i} style={{padding:4,minHeight:90,borderBottom:“1px solid “+K.brd,borderRight:“1px solid “+K.brd,background:“rgba(0,0,0,.15)”}}/>;})}
{Array.from({length:dm},function(*,i){
