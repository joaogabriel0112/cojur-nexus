...
const SB_URL = "https://vcxastdcsbzdsfcdbtan.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZjeGFzdGRjc2J6ZHNmY2RidGFuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU3MjEzMTEsImV4cCI6MjA5MTI5NzMxMX0.DooE1OfcYeDnTor1d40N6ndTnUtP3U7waNkB8tqQn-k";
const SB_USER = "joao_gabriel_cojur";
...
var dbGet = async function(){
  // localStorage first (instantaneous)
  var local = null;
  try { local = localStorage.getItem(STORE_KEY); } catch(e) {}
  // Try Supabase
  try {
    var r = await Promise.race([
      fetch(SB_URL+"/rest/v1/nexus_state?user_id=eq."+SB_USER+"&select=data", {headers: SB_HEADERS}),
      new Promise(function(res){ setTimeout(function(){ res(null); }, 3000); })
    ]);
    if (r && r.ok) {
      var rows = await r.json();
      if (rows && rows.length > 0 && rows[0].data) {
        try { localStorage.setItem(STORE_KEY, rows[0].data); } catch(e) {}
        return rows[0].data;
      }
    }
  } catch(e) {}
...
var dbSet = async function(value){
  ...
  fetch(SB_URL+"/rest/v1/nexus_state", {
    method: "POST",
    headers: SB_HEADERS,
    body: JSON.stringify({user_id: SB_USER, data: value, updated_at: new Date().toISOString()})
  }).catch(function(){});
};
